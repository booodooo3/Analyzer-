...
export const ImageUploader: React.FC<UploaderProps> = ({ description, currentImage, currentUrl, onImageSelected, className, objectFit = 'cover' }) => {
  ...
  return (
    <div 
      ...
      className={`border-2 border-dashed border-zinc-800 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:border-zinc-600 hover:bg-zinc-900/50 transition-all relative overflow-hidden group ${className || 'aspect-[3/2] max-h-64'}`}
    >
      {displayImage ? (
        <>
