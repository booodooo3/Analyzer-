...
app.post('/api/generate', ClerkExpressWithAuth(), async (req: any, res: any) => {
    try {
        ...
        if (isPlusModeBool) {
             ...
             const prompts = [
                { type: 'front', text: `Medium shot of a person wearing ${desc}. Strictly framing from the top of the head to the upper thighs. The frame MUST cut off at the hip line or upper thighs. Do NOT show the knees, legs, or feet. Crop strictly at the upper thigh level. High quality, realistic. Preserve the original person's face and identity with 100% accuracy, perform the swap only on the garment regions. The face, eyes, nose, lips, and hair are reference-locked. They must be a 1:1 match with the input photo.` },
