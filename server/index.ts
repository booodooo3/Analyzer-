...
app.post('/api/generate', ClerkExpressWithAuth(), async (req: any, res: any) => {
    try {
        ...
        if (isPlusModeBool) {
             ...
             const prompts = [
                { type: 'front', text: `Medium shot, waist-up front view of a person wearing ${desc}. The frame cuts off at the waist/stomach. The person is wearing the garment shown in the second image. High quality, realistic. Preserve the original person's face and identity with 100% accuracy, perform the swap only on the garment regions. The face, eyes, nose, lips, and hair are reference-locked. They must be a 1:1 match with the input photo.` },
